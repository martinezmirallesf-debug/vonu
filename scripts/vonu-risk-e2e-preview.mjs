...
const corpus = JSON.parse(fs.readFileSync("scripts/fixtures/vonu-risk-corpus.json", "utf8"));
...
try {
  ...
  for (const testCase of corpus.textCases) {
    ...
    results.push(result);
    console.log(`VONU_RISK_E2E_CASE ${JSON.stringify(result)}`);
  }

  const total = corpus.textCases.length;
  if (passed !== total) {
    console.error(`VONU_RISK_E2E_RED passed=${passed} total=${total} failed=${total - passed}`);
    process.exitCode = 1;
  } else {
    console.log(`VONU_RISK_E2E_GREEN passed=${passed} total=${total}`);
  }
} finally {
