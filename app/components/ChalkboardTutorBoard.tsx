...
console.log("✅ ChalkboardTutorBoard ACTIVO (layout 50/50 + sanitize + clip + image + no-overlap)");
...
// ================== SANITIZE ==================
/**
 * Limpia restos de prompts / tool json / tags raras.
 * Si viene "demasiado jsony" colado, mejor devolver vacío.
 */
function sanitizeBoardValue(raw: string) {
  ...
  for (const line of lines) {
    ...
    if (!t) continue;
    if (t.includes("{{IM") || t.includes("[[IM") || t.includes("placement") || t.includes("boardImage") || t.includes("b64"))
      continue;
  ...
  useEffect(() => {
    ...
    const draw = async () => {
      ...
      if (auto?.layout === "twoCol") {
        ...
        const leftTitle = (auto.leftTitle || "IZQ").trim();
        const rightTitle = (auto.rightTitle || "DER").trim();
        ...
        // ✅ TAMAÑOS más “clase” (menos título gigante)
        const TITLE_SIZE = 56;
        const H_SIZE = 32;
        const ITEM_SIZE = 34;
        ...
        if (title) {
          ...
          // underline
          const ulY = y + TITLE_SIZE + 8;
          drawWobblyLine(ctx, leftX, ulY, leftX + Math.min(520, colW - 28), ulY, COLOR_MAP.yellow, 6, rng);
        ...
        // ✅ Si hay imagen en la zona derecha, reservamos espacio y bajamos el texto derecho
        // para evitar cualquier solape.
        let rightStartY = y;
        if (imgRect) {
          const rightColLeft = rightX;
          const rightColRight = rightX + colW - 24;

          const imgLeft = imgRect.x;
          const imgRight = imgRect.x + imgRect.w;

          const overlapsRightCol = imgRight > rightColLeft && imgLeft < rightColRight;
          if (overlapsRightCol) {
            rightStartY = Math.max(rightStartY, imgRect.y + imgRect.h + 16);
          }
        }
        ...
        for (let i = 0; i < rows; i++) {
          const ly = y + i * rowH;
          const ry = rightStartY + i * rowH;

          const ltxt = left[i] || "";
          const rtxt = right[i] || "";

          if (ltxt) drawText(ctx, ltxt, leftX, ly, ITEM_SIZE, COLOR_MAP.cyan, rng);
          if (rtxt) drawText(ctx, rtxt, rightX, ry, ITEM_SIZE, COLOR_MAP.green, rng);

          // flecha SOLO si hay izquierda y derecha
          if (ltxt && rtxt) {
            const ax1 = leftX + colW - 44;
            const ax2 = rightX - 20;
            const ay = ly + 18;
        ...
        // note
        if (note) {
          const ny = Math.max(y + rows * rowH + 12, rightStartY + rows * rowH + 12);
      ...
      const title = lines[0] || "";
      if (title) {
        drawText(ctx, title.toUpperCase(), x, y, 56, COLOR_MAP.white, r);
        const ulY = y + 56 + 8;
