...
export default function AdvancedVonuThinking({ size = 32 }: VonuThinkingProps) {
  return (
    <span
      ...
      aria-hidden="true"
    >
      {/* NÚCLEO BIOMÓRFICO: Capas de plasma líquido que mutan de forma independiente */}
      <span className="vonu-plasma vonu-plasma-1" />
      <span className="vonu-plasma vonu-plasma-2" />
      <span className="vonu-plasma vonu-plasma-3" />
      
      {/* AURA CUÁNTICA: Destellos radiales de alta frecuencia */}
      ...
      <style jsx>{`
        ...
        /* --- 1. EL PLASMA (FLUIDO INTELIGENTE) --- */
        .vonu-plasma {
          position: absolute;
          inset: -50%;
          border-radius: 40% 60% 70% 30% / 40% 50% 60% 50%; /* Geometría fluida inicial */
          filter: blur(calc(var(--v-size) * 0.28));
          mix-blend-mode: screen;
          will-change: transform, border-radius;
        }

        /* Capa Cian/Azul Cósmico - Movimiento base pesado */
        .vonu-plasma-1 {
          background: conic-gradient(from 0deg, #4285f4, #1a73e8, transparent, #4285f4);
          animation: 
            morphFluid 7000ms ease-in-out infinite alternate,
            rotateFluid 11000ms linear infinite;
          opacity: 0.55;
        }

        /* Capa Aurora Verde/Neon - Contrapeso rápido */
        .vonu-plasma-2 {
          background: conic-gradient(from 120deg, #34a853, #00f2fe, transparent, #34a853);
          animation: 
            morphFluid 5000ms ease-in-out infinite alternate-reverse,
            rotateFluid 8000ms linear infinite reverse;
          opacity: 0.45;
          scale: 0.95;
        }

        /* Capa Destello Cálido (Magenta/Oro) - Pulsos de consciencia */
        .vonu-plasma-3 {
          background: radial-gradient(circle, #ea4335 0%, #fbbc05 40%, transparent 70%);
          animation: 
            quantumPulse 3300ms cubic-bezier(0.4, 0, 0.2, 1) infinite;
          opacity: 0.35;
        }

        /* --- 2. AURA CUÁNTICA (NEBULA PERIFÉRICA) --- */
        .vonu-quantum-aura {
          position: absolute;
          inset: -30%;
          border-radius: 50%;
        ...
        /* --- 3. CONTENEDOR DE LOGO CON REFRACCIÓN CRISTALINA --- */
        .vonu-logo-wrapper {
          position: absolute;
          inset: 0;
          display: flex;
        ...
        /* El efecto Cromático: Un clon del logo desfasado sutilmente que brilla por detrás */
        .vonu-logo-chromatic {
          position: absolute;
          inset: -2px;
          background-image: var(--v-logo);
        ...
        /* --- 4. ANIMACIONES AVANZADAS (60FPS - GPU DRIVEN) --- */
        
        /* Simula la mutación de una célula líquida de energía */
        @keyframes morphFluid {
          ...
          100% {
            border-radius: 50% 50% 30% 70% / 35% 65% 35% 65%;
        ...
        /* Pulso hiper-suave de energía pura */
        ...
        /* Flotación micro-espacial para el logo */
        @keyframes logoFloat {
          0%, 100% { transform: translateY(0) scale(0.96); }
          50% { transform: translateY(-3%) scale(1.02); }
        }

        /* Destellos intermitentes en las aristas del logo (Efecto Computacional) */
        ...
        @media (prefers-reduced-motion: reduce) {
          .vonu-plasma, .vonu-quantum-aura, .vonu-logo-wrapper, .vonu-logo-chromatic {
