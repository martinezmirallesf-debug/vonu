...
export async function POST(req: Request) {
  try {
    ...
    if (!webhookSecret) {
      return NextResponse.json({ error: "Missing STRIPE_WEBHOOK_SECRET in env" }, { status: 500 });
    }
    ...
    async function grantDevicePack(session: Stripe.Checkout.Session) {
      const metadata = session.metadata || {};
      const deviceId = metadata.device_id || null;
      const analyses = Number(metadata.analyses || 0);
      const currency = (session.currency || "").toLowerCase();
      const amountTotal = session.amount_total ?? null;
      const paid = session.payment_status === "paid" || session.payment_status === "no_payment_required";

      if (
        metadata.kind !== "device_pack" ||
        !paid ||
        !isUuid(deviceId) ||
        analyses !== 3 ||
        currency !== "eur" ||
        ...
        return { handled: metadata.kind === "device_pack", granted: false };
      }
      ...
      const { data, error } = await supabaseAdmin.rpc("grant_vonu_device_pack", {
    ...
    async function upsertSubscriptionRow(params: {
      ...
      if (!userId) userId = await resolveUserIdFromCustomer(params.stripeCustomerId);
      if (!userId) userId = await getExistingUserIdBySubscription(params.stripeSubscriptionId);
    ...
    async function syncProfilePlan(params: {
      ...
      const isActive = params.subscriptionStatus === "active" || params.subscriptionStatus === "trialing";
      const nextPlan = isActive ? params.appPlan ?? "free" : "free";
      const { error } = await supabaseAdmin
        .from("profiles")
        .upsert({ id: params.userId, plan: nextPlan, updated_at: new Date().toISOString() }, { onConflict: "id" });
      if (error) throw new Error(error.message);
    }

    function getAppPlanFromPriceId(priceId: string | null): "plus" | "max" | null {
      if (!priceId) return null;
      const plusPrices = [process.env.STRIPE_PRICE_PLUS_MONTHLY, process.env.STRIPE_PRICE_PLUS_YEARLY].filter(Boolean);
      const maxPrices = [process.env.STRIPE_PRICE_MAX_MONTHLY, process.env.STRIPE_PRICE_MAX_YEARLY].filter(Boolean);
      if (plusPrices.includes(priceId)) return "plus";
      if (maxPrices.includes(priceId)) return "max";
      return null;
    }

    async function insertTopup(params: { userId: string; pack: string | null; stripeSessionId: string | null }) {
    ...
    switch (event.type) {
      case "checkout.session.async_payment_succeeded": {
        ...
        const devicePack = await grantDevicePack(session);
        if (devicePack.handled) {
          return NextResponse.json({ ok: true, type: "device_pack", ...devicePack });
        }
      ...
      case "checkout.session.completed": {
        ...
        if (kind === "topup") {
          ...
          const userId =
            userIdFromMeta ||
            (await resolveUserIdFromCustomer(typeof session.customer === "string" ? session.customer : null));
          if (userId) {
            await insertTopup({ userId, pack, stripeSessionId: session.id });
          }
          return NextResponse.json({ ok: true, type: "topup" });
        }
        ...
        await syncProfilePlan({ userId: resolvedUserId, subscriptionStatus: sub.status ?? null, appPlan });
      ...
      case "customer.subscription.deleted": {
        ...
        await syncProfilePlan({ userId: resolvedUserId, subscriptionStatus: sub.status ?? null, appPlan });
        return NextResponse.json({ ok: true, type: "subscription_update" });
      }
