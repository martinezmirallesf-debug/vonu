...
const PLAN_PREF_KEY = "vonu_plan_pref_v1"; // "free" | "monthly" | "yearly"
...
export default function Page() {
  ...
  const [currentPlanLabel, setCurrentPlanLabel] = useState<
    "Free" | "Pro Monthly" | "Pro Yearly" | "Pro"
  >("Free");

  function readPlanPref(): "free" | "monthly" | "yearly" {
    try {
      const v = window.localStorage.getItem(PLAN_PREF_KEY);
      if (v === "monthly" || v === "yearly" || v === "free") return v;
    } catch {
      // ignore
    }
    return "free";
  ...
  function writePlanPref(v: "free" | "monthly" | "yearly") {
    try {
      window.localStorage.setItem(PLAN_PREF_KEY, v);
    } catch {
      // ignore
    }
  }

  function computeCurrentPlanLabel(active: boolean): "Free" | "Pro Monthly" | "Pro Yearly" | "Pro" {
    if (!active) return "Free";
    const pref = mounted ? readPlanPref() : "monthly";
    if (pref === "monthly") return "Pro Monthly";
    if (pref === "yearly") return "Pro Yearly";
    return "Pro";
  }
  ...
  async function refreshProStatus() {
    ...
    try {
      ...
      setIsPro(active);
      setCurrentPlanLabel(computeCurrentPlanLabel(active));
    } catch {
      setIsPro(false);
      setCurrentPlanLabel("Free");
    } finally {
  ...
  useEffect(() => {
    if (!mounted) return;
    // cargar preferencia local para que el selector abra donde toca
    setPlan(readPlanPref());
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mounted]);
  ...
  async function startCheckout(chosen: "monthly" | "yearly") {
    ...
    // guardamos para mostrar luego el plan actual
    writePlanPref(chosen);
  ...
  const isPaywalled =
    !authLoading && !!authUserId && !proLoading && !isPro; // ahora: si está logueado pero no Pro => bloqueado

  const canSend = useMemo(() => {
    const basicReady = !isTyping && (!!input.trim() || !!imagePreview);
    if (isPaywalled) return false;
    return basicReady;
  }, [isTyping, input, imagePreview, isPaywalled]);
  ...
  const priceMonthlyLabel = "4,99€";
  const priceYearlyLabel = "39,99€";

  const pillPlan =
    currentPlanLabel === "Free"
      ? "Free"
      : currentPlanLabel === "Pro Monthly"
      ? "Pro · Monthly"
      : currentPlanLabel === "Pro Yearly"
      ? "Pro · Yearly"
      : "Pro";
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-zinc-500 mt-1">
                  Elige tu plan y gestiona tu suscripción cuando quieras.
                </div>
              </div>

              <div className="flex items-center gap-2">
                <span className="h-8 px-3 rounded-full bg-blue-50 border border-blue-100 text-[12px] text-blue-700 inline-flex items-center gap-1">
                  <span className="font-semibold">Plan actual:</span> {pillPlan}
                </span>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-2">
              ...
              <button
                ...
                className={[
                  ...
                  plan === "free"
                    ? "border-blue-600 bg-blue-600 text-white shadow-[0_12px_30px_rgba(66,133,244,0.25)]"
              ...
              >
                ...
                <div className="mt-1 text-2xl font-semibold leading-none">
                  0€
                  <span className={plan === "free" ? "text-white/80" : "text-zinc-500"}> /siempre</span>
                </div>
                <div className={plan === "free" ? "text-white/85 text-xs mt-1" : "text-zinc-500 text-xs mt-1"}>
                  Para probar y quedarte tranquilo
                </div>
              </button>
              ...
              <button
                onClick={() => {
                  setPlan("monthly");
                  writePlanPref("monthly");
                  setPayMsg(null);
                }}
                className={[
                  "rounded-3xl border p-4 text-left transition-all cursor-pointer",
                  plan === "monthly"
                    ? "border-blue-600 bg-blue-50 shadow-[0_12px_30px_rgba(66,133,244,0.12)]"
                    : "border-zinc-200 bg-white hover:bg-zinc-50",
                ].join(" ")}
                disabled={payLoading}
              >
                <div className="text-xs font-semibold text-zinc-900">Monthly</div>
                <div className="mt-1 text-2xl font-semibold leading-none text-zinc-900">
                  {priceMonthlyLabel}
                  <span className="text-zinc-500"> /mes</span>
                </div>
                <div className="text-zinc-500 text-xs mt-1">Flexible, cancela cuando quieras</div>
              </button>
              ...
              <button
                onClick={() => {
                  setPlan("yearly");
                  writePlanPref("yearly");
                  setPayMsg(null);
                }}
                className={[
                  "rounded-3xl border p-4 text-left transition-all cursor-pointer relative overflow-hidden",
                  plan === "yearly"
                    ? "border-blue-700 bg-blue-600 text-white shadow-[0_12px_30px_rgba(66,133,244,0.28)]"
                    : "border-zinc-200 bg-white hover:bg-zinc-50",
                ].join(" ")}
                disabled={payLoading}
              >
                <div className="absolute top-3 right-3">
                  <span
                    className={[
                      "text-[11px] px-2 py-1 rounded-full inline-flex items-center gap-1",
                      plan === "yearly" ? "bg-white/15 text-white border border-white/25" : "bg-blue-50 text-blue-700 border border-blue-100",
                    ].join(" ")}
                  >
                ...
                <div className="text-xs font-semibold">Yearly</div>
                <div className="mt-1 text-2xl font-semibold leading-none">
                  {priceYearlyLabel}
                  <span className={plan === "yearly" ? "text-white/80" : "text-zinc-500"}> /año</span>
                </div>
                <div className={plan === "yearly" ? "text-white/85 text-xs mt-1" : "text-zinc-500 text-xs mt-1"}>
            ...
            <div className="mt-4 flex flex-col md:flex-row gap-2 md:items-center md:justify-between">
              ...
              {plan === "free" ? (
                <button
                  ...
                  className="h-11 px-5 rounded-full text-sm cursor-pointer transition-colors bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50"
                  disabled={payLoading}
                >
                  Seguir en Free
                </button>
                    ...
                    {authUserId && (
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-[11px] text-zinc-500">
                          Estado: {proLoading ? "comprobando…" : isPro ? pillPlan : "Free"}
                        </div>
        ...
        <div ref={inputBarRef} className="sticky bottom-0 left-0 right-0 z-30 bg-white/92 backdrop-blur-xl">
          <div className="mx-auto max-w-3xl px-3 md:px-6 pt-3 pb-2 flex items-end gap-2 md:gap-3">
            ...
            <button
              ...
              disabled={isTyping || isPaywalled}
              title={
                isTyping ? "Espera a que Vonu responda…" : isPaywalled ? "Disponible en Pro" : "Adjuntar imagen"
              }
            >
