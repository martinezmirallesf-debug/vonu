...
function OAuthLogo({ src, alt, invert }: { src: string; alt: string; invert?: boolean }) {
  return (
    <img
      ...
      alt={alt}
      className="h-5 w-5"
      style={{ display: "block", filter: invert ? "invert(1)" : undefined }}
      draggable={false}
    />
  );
}
...
export default function Page() {
  ...
  // ✅ FIX OAuth: usar URL canónica de la app (NO window.location.origin)
  async function signInWithOAuth(provider: "google" | "azure") {
    setLoginSending(true);
    setLoginMsg(null);
    try {
      const siteUrl = ((process.env.NEXT_PUBLIC_SITE_URL as string | undefined) || "https://app.vonuai.com").replace(/\/$/, "");
        ...
        <div ref={inputBarRef} className="sticky bottom-0 left-0 right-0 z-30 bg-white/92 backdrop-blur-xl">
          <div className="mx-auto max-w-3xl px-3 md:px-6 pt-3 pb-2 flex items-end gap-2 md:gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className={["w-full min-h-11 md:min-h-12 px-4 py-3 flex items-center", "bg-zinc-100", "border border-zinc-200 focus-within:border-zinc-300", inputExpanded ? "rounded-3xl" : "rounded-full"].join(" ")}>
                <textarea
                  ref={textareaRef}
                  value={input}
