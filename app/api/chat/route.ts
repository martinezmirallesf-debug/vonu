...
function cleanTutorLevel(level: unknown): TutorLevel {
  if (level === "kid" || level === "teen" || level === "adult" || level === "unknown") {
    return level;
  }
...
function cleanImageThumb(imageThumb: unknown) {
  ...
  // Seguridad extra: evitamos guardar miniaturas enormes.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Guardado simple y robusto:
    // reemplazamos los mensajes del thread por la versión actual.
    ...
    const rows = messages
      ...
      .map((message, index) => ({
        ...
        pizarra:
          typeof message.pizarra === "string" && message.pizarra.trim()
            ? message.pizarra
            : null,
